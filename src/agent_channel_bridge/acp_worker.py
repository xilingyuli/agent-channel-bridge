...
class AcpWorker:
    ...
    async def _do_send_prompt(self, sid: str, route_key: str, text: str, qq_msg: dict = None):
        ...
        ctx_lines.append("  1. 开始执行重要工具/命令前 — 告诉用户你要做什么")
        ctx_lines.append("  2. 用户指令执行完毕时 — 告知结果或下一步")
        ctx_lines.append("  3. 工具执行慢或卡住时 — 告知用户 '正在执行，请稍候'")
        ctx_lines.append("  4. 遇到问题需要用户决策时 — 询问意见")
        ctx_lines.append("  5. 长时间无文字输出时（超过10秒）— 必须输出一条进度")
        ctx_lines.append("")
